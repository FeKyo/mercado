...
// ─── Hardware tokens ──────────────────────────────────────────────────────────
// Tokens fixos para dispositivos físicos (ESP8266)
// O gerente gera um token no painel e cola no código do ESP8266

// POST /api/hardware/token  — gerente gera token para um dispositivo
app.post('/api/hardware/token', managerOnly, (req, res) => {
  ...
  db.prepare('INSERT INTO hardware_tokens (token, label, created_by) VALUES (?,?,?)')
    .run(token, label||'ESP8266', req.user.id);
  res.json({ token });
});

// GET /api/hardware/tokens  — lista tokens do gerente
app.get('/api/hardware/tokens', managerOnly, (req, res) => {
  try {
    const rows = db.prepare(`
      SELECT id, label, substr(token,1,8)||'...' as token_preview, created_at
      FROM hardware_tokens WHERE created_by = ? ORDER BY created_at DESC
    `).all(req.user.id);
    res.json(rows);
  } catch { res.json([]); }
});
...
app.delete('/api/hardware/tokens/:id', managerOnly, (req, res) => {
  db.prepare('DELETE FROM hardware_tokens WHERE id=? AND created_by=?')
    .run(req.params.id, req.user.id);
  res.json({ ok: true });
});

// Middleware para autenticar hardware
function hwAuth(req, res, next) {
  const h = req.headers.authorization;
  if (!h?.startsWith('Bearer ')) return res.status(401).json({ error: 'Token ausente' });
  const token = h.slice(7);
  try {
    // Tenta como JWT primeiro (gerente logado testando via curl)
    req.user = jwt.verify(token, SECRET);
    return next();
  } catch {}
  // Tenta como hardware token
  try {
    const row = db.prepare('SELECT * FROM hardware_tokens WHERE token=?').get(token);
...
// POST /api/shelves/weight  — ESP8266 envia leituras
app.post('/api/shelves/weight', hwAuth, (req, res) => {
  db.exec(`
    CREATE TABLE IF NOT EXISTS shelf_weights (
      id        INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  const insert = db.prepare(
    'INSERT INTO shelf_weights (shelf_id, slot, peso_g, pct) VALUES (?,?,?,?)'
  );
  const tx = db.transaction(() => {
    slots.forEach(s => insert.run(shelf_id, s.slot, s.peso ?? 0, s.pct ?? 0));
  });
  tx();
  res.json({ ok: true, recorded: slots.length });
});

// GET /api/shelves/status  — frontend consulta estado atual de todas as prateleiras
app.get('/api/shelves/status', (req, res) => {
  try {
    // Última leitura de cada shelf_id + slot
    const rows = db.prepare(`
